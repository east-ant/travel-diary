...
// 우정여행 전용 레이아웃 (이미지 기반)
export function FriendTravelPrint({
  photoSlots,
  diaryText,
  title,
  onBack,
        ...
        <div className="flex-1 space-y-8">
          {pages.map((page, pageIdx) => (
            ...
            >
              ...
              {/* 메인 콘텐츠: 각 사진마다 옆에 일기 배치 */}
              <div className="space-y-8">
                {page.photos.map((photo, idx) => {
                  const imageUrl = getImageUrl(photo)
                  const text = page.texts[idx]
                  const isLastPage = pageIdx === pages.length - 1
                  const isLastItem = isLastPage && idx === page.photos.length - 1
